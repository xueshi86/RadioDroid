...
- 提供**增量更新**：基于 `json/stations/lastchange` 端点仅拉取自上次同步以来变更的电台（每次批量 1000 条，uuid 水位 + REPLACE 主库直写），秒级完成、流量消耗极小；可在设置中手动触发，或开启「自动增量更新」（默认关闭；开启后距上次更新 24 小时静默执行，可限制仅 Wi-Fi）

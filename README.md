...
- 提供 **WebDAV 备份和恢复**：设置中新增「备份和恢复」子页，配置 WebDAV 服务器（服务器地址、服务器目录、账号、密码四项；密码经 Android Keystore 加密存储，HTTP 地址提示明文风险）后，可将**收藏电台**（M3U）与**离线数据库**（SQLite）单独或同时备份到远端、从远端恢复；备份文件存放于「服务器地址/服务器目录/」下（目录可留空表示根目录，不存在时上传自动逐级创建 MKCOL）；配置摘要带连通性状态灯（检测中灰色/绿色交替闪烁，成功绿色，失败红色，未配置灰色）；任务进行中在设置页显示，完成后展示收藏/数据库分项结果；数据库恢复前先下载校验（SQLite 文件头、完整性、表结构）并需前台确认，失败自动回滚保留原库
...
| 数据时效性  | 增量更新大幅缓解：应用启动时若距上次更新超 24h 自动同步，手动增量秒级完成 |
| 存储空间   | 本地数据库约 40MB                       |
| ️ 更新方式 | 支持手动全量 / 手动增量 / 自动增量（默认关闭，可仅 Wi-Fi）    |
...
1. **优先显示系统国家电台**：根据手机系统国家代码，从数据库筛选该国电台。国家代码通过回退链获取——优先取系统 `Locale` 的国家代码，若未设置国家（返回空值）则依次回退到 **SIM 卡国家**、**运营商网络注册国家**（`getSimCountryIso`/`getNetworkCountryIso`），跨时区/无实体 SIM 的场合也能尽量定位「本地」，不再因 `Locale` 无国家代码而直接跳过他国匹配。
2. **回退到系统语言电台**：若国家无电台（或国家代码确实缺失），尝试查询系统语言+国家的电台组合。
3. **进一步回退**：仍无结果则查询仅按语言筛选。
4. **兜底显示全部电台**：所有条件均不满足时显示全部电台列表。

> **国家/语言匹配口径**：本地数据库 `countrycode` 存 ISO 3166-1 大写代码（如 `CN`），与系统国家代码大小写一致可精确匹配；`language` 字段存 radio-browser 的英文全名（如 `english,german`），因此查询时先**将系统语言 ISO 码映射为英文全名**（`en→english`、`zh→chinese`…），并按逗号分词做包含匹配（`LIKE` 命中任一语言），避免 `en` = `english,german` 恒为空导致的语言降级失效。

刷新列表时始终遵循此优先级逻辑，确保用户首先看到最可能感兴趣的电台；闹钟等「无播放历史自动选台」的兜底逻辑与列表页保持一致，同样应用上述国家回退与语言映射。
...
- WebDAV backup and restore: a dedicated Settings → Backup and Restore section (four fields: server address, server directory, username, password; Basic auth, password encrypted via Android Keystore, with a plaintext warning for HTTP addresses) backs up and restores favourites (M3U) and the offline database (SQLite) individually or together; backup files are stored under "server address/server directory/" (directory is optional and auto-created via MKCOL on upload); the configuration entry shows a connectivity indicator (gray/green blinking while checking, green on success, red on failure, gray when not configured); tasks show an in-progress state and report per-item results for favourites and the database; database restores are downloaded and validated (SQLite header, integrity, schema) first and require foreground confirmation, with automatic rollback to the original database on failure

...
- **新增**：设置中新增「备份和恢复」子页，包含 WebDAV 配置、备份、恢复三个入口；配置页提供服务器地址、账号、密码三行输入与删除/保存按钮，服务器地址直接填写远端目录 URL（如 `https://host/dav/radiodroid/`），已有配置密码留空表示沿用原密码
- **新增**：备份/恢复均支持三种模式 — 收藏电台（M3U）、离线数据库（SQLite）、两者；远端使用固定文件名 `favourites.m3u` 与 `radio_droid_database.db`，备份覆盖同名远端文件；收藏恢复为覆盖式（与 M3U 导入一致），空收藏备份文件可恢复为空收藏；`两者` 模式下两项独立执行并分项报告结果

...
- **修复**：`/json/countries` 中同一国家可能存在大小写不同的重复条目（如 `US` 与 `us`），统一转大写后美国被重复下载、下载计数虚高（显示 73,221，实际全部电台约 65,036）；现按大写码去重，下载计数与实际一致
...
- **实现**：`updater` 包新增 `UpdateChecker`（版本查询/解析/数值逐段比较/按 flavor 匹配 APK asset）、`UpdateDownloader`（流式下载 + 进度通知 + 文件提供器唤起安装）、`UpdateReceiver`；`RadioDroidApp` 新增 `maybeAutoCheckUpdate()` 沿用自动增量更新模式的节流与网络判断
...
- **实现**：Room DAO 新增按电台过滤的 `DataSource.Factory` 查询，ViewModel 通过 `switchMap` 实现按电台动态切换数据源；「本台曲目」标签名支持中文、英文、俄语、西班牙语、德语、法语、意大利语、希腊语 8 种语言
- **变更**：两个曲目历史标签的列表项新增播放日期时间显示——置于列表项最右方并垂直居中，组合串较短时单行显示，过长时自动分两行（日期一行、时间一行），曲目名/艺术家文本自动避让
...
- **新增**：**LrcAPI** 聚合歌词来源（[HisAtri/LrcApi](https://github.com/HisAtri/LrcApi)，公共实例 api.lrc.cx）作为第二回退源——`/lyrics` 端点按 title/artist 查询、直接返回 LRC 文本，自动按时间标签判定同步/纯文本歌词；设置中支持自建实例地址（留空使用公共实例）
- **变更**：**网易云**歌词来源改为默认开启，作为 LRCLIB 和 LrcAPI 之后的第三回退源，提升中文曲目命中率；搜索与歌词接口均为非官方公开 API（请求带 Referer），可在设置中手动关闭
- **新增**：歌词 Room 缓存——新增 `lyrics_cache` 表（数据库 v14→v15 显式迁移保护用户数据），按「艺术家+曲目」为键缓存 30 天，减少重复网络请求
- **新增**：设置「播放」下方新增「歌词」分类——来源模式（内置来源 / 外部歌词应用，默认内置）、LRCLIB 实例地址、LrcAPI 实例地址、网易云开关（默认开启）；保留「外部歌词应用」模式兼容原 QuickLyric Intent 调用协议，未安装兼容应用时给出明确提示，不再引导到已下架的死链下载页
- **变更**：选择「外部歌词应用」模式后，内置来源的 LRCLIB/LrcAPI 实例地址与网易云开关自动禁用（进入设置页与切换模式时均实时生效），避免无效配置误导用户
- **新增**：歌词设置页顶部新增说明文案，向用户介绍内置来源降级顺序（LRCLIB → LrcAPI → 网易云，首个命中者生效）、自建实例配置方式（填完整 URL 如 https://host:port/path，留空使用公共默认接口）与外部模式行为；LRCLIB/LrcAPI 实例地址的说明同步补充 URL 格式与默认值提示；文案适配中文、英文、俄语、西班牙语、德语、法语、意大利语、希腊语 8 种语言
- **优化**：针对电台元数据常带「歌手 - 歌名 (Live)」式括号污染，获取歌词前做多查询组合（剥离括号、按「 - 」拆分）提升命中率并降低误匹配；歌词结果含时长与曲名相似度打分
...
- **改进**：「资源未找到」错误文案更具体 —— 明确为「服务器上的目录或文件不存在，请先执行一次备份」（`webdav_error_not_found`），避免恢复收藏前被误导去改目录配置
- **新增**：收藏恢复增加「覆盖 / 合并」选择 —— 恢复收藏前弹出确认对话框（新增 `webdav_restore_favourites_confirm`/`webdav_restore_overwrite`/`webdav_restore_merge`）：覆盖则本地列表与服务器完全一致；合并则保留全部本地电台并按 UUID 去重追加服务器独有电台、保持本地顺序；数据库恢复流程不变
- **新增**：恢复操作前的本地快照保护 —— 覆盖或合并收藏前，先将当前收藏导出为本地快照（缓存目录 `favourites_local_backup.m3u`），误操作后可手动恢复，防止丢失收藏
- **实现**：`StationSaveManager` 新增 `mergeMultiple()` 合并导入（与整体替换语义的 `addMultiple()` 区分，M3U/OPML/JSON 文件导入行为不变）；`WebDavBackupWorker` 通过任务参数传递恢复模式
- **测试**：WebDAV 单元测试迁移至 `ensureConnection` 语义并扩充 —— 覆盖「404/409 → 逐级 MKCOL → PROPFIND 重试」成功路径、多级目录逐段创建、创建后仍缺失报 NOT_FOUND、创建期间认证失败报 AUTHENTICATION 等分支
- **多语言**：上述界面与提示文案覆盖中文、英文、俄语、西班牙语、德语、法语、意大利语、希腊语 8 种语言
